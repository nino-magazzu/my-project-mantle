...
public class ListOfFileDownloader extends AsyncTask<Void, Entry, Descriptor[]>{
	...
	public ListOfFileDownloader(DropboxAPI<AndroidAuthSession> mApi, Context cont) {
		this.mApi = mApi;
		this.context = cont;
	}
	...
	protected Descriptor[] doInBackground(Void... params) {
        ...
        /* public DropboxAPI.Entry metadata(java.lang.String path,
                             int fileLimit,
           ...
           Parameters:
			...
			fileLimit - the maximum number of children to return for a directory. Default is 25,000 if you pass in 0 or less. 
						If there are too many entries to return, you will get a 406 DropboxServerException. 
						Pass in 1 if getting metadata for a file.
						
			hash - 	if you previously got metadata for a directory and have it stored, pass in the returned hash. 
					If the directory has not changed since you got the hash, a 304 DropboxServerException will be thrown. 
					Pass in null for files or unknown directories.
					
			list - if true, returns metadata for a directory's immediate children, or just the directory entry itself if false. 
					Ignored for files.
	...
	try {
			...
			for (Entry ent: dirent.contents) {
				...
				if(ent.isDir) {
					...
					dir.get(dir.size() - 1).setFiles(temp.toArray(new Descriptor[temp.size()]));
				}
				else
					continue;
			}
